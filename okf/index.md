...
tags: [cad, occt, viewport, metal, swiftui, step, stl, brep, face-picking, edge-picking, vertex-picking, durable-identity, multi-body, assembly, kernel]
description: Shared SwiftUI Metal CAD viewport plus STEP/STL/BREP import (single-shape or multi-body/assembly) and face/edge/vertex picking for OCCT-based apps.
...
> coexisting, addressable entities for an assembly) and face/edge/vertex picking, for apps built on
> OCCTSwift and OCCTSwiftViewport. Extracted from PadCAM's `CADViewportService` / `CADViewportView`
> so multiple OCCT-based apps can share the same viewport plumbing without forking it.
