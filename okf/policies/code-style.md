...
`docs/reference/` documents this repo's public Swift surface (`CADViewportService` and similar);
keep it current per [docs-current](docs-current.md) rather than letting source doc comments
restate it.
...
**No `clang-format` here.** This repo carries zero first-party C++ bridge files (confirmed at
rollout), so the wider proposal's C++ half doesn't apply.

Why: this repo followed OCCTSwiftScripts' pilot in the ecosystem's rollout order, small enough
(~5,644 Swift lines across ~13 files) to sweep into full compliance in one PR rather than needing
a gradual adoption mechanism. Full research and rationale:
...
[OCCTSwiftCADKit#56](https://github.com/SecondMouseAU/OCCTSwiftCADKit/issues/56), following the
reference implementation in
[OCCTSwiftScripts#114](https://github.com/SecondMouseAU/OCCTSwiftScripts/issues/114) /
