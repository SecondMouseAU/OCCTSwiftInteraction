...
("share the link to the documentation instead of rephrasing the information"). This repo's own
`docs/reference/` documents this repo's Swift declarations directly (one page per converter/table
type), so the same doc comment/reference-page duplication risk the wider proposal identifies in
`OCCTSwift` applies here too; the mechanical docs-drift checks proposed for `OCCTSwift`
(`check-docs-completeness.py` and friends) are explicitly out of scope for this rollout, tracked
separately if this repo's docs drift becomes a live problem.
...
Why: this repo is small enough (~19 files, ~2,280 Swift lines, all pure Swift, zero first-party
C++ bridge files) to sweep into full compliance in one PR rather than needing a gradual adoption
mechanism, the same reasoning [OCCTSwiftScripts](https://github.com/SecondMouseAU/OCCTSwiftScripts)
used as the ecosystem's pilot repo. Full research and rationale:
...
Reference implementation: [OCCTSwiftScripts#114](https://github.com/SecondMouseAU/OCCTSwiftScripts/issues/114) /
[#115](https://github.com/SecondMouseAU/OCCTSwiftScripts/pull/115).
Filed and tracked as [OCCTSwiftTools#53](https://github.com/SecondMouseAU/OCCTSwiftTools/issues/53).
