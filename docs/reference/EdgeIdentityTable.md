...
- An element is `nil` when that edge's `Edge` to `Shape` conversion failed. See [ordinal alignment](ShapeIdentity#ordinal-alignment-is-the-invariant) for why the array is optional rather than short.
- `uids` is populated only when a `BRepGraph` was supplied to the entry point that produced this table. Each element is `nil` if that ordinal's edge could not be resolved in the graph, or has no entry in `shapes`.
